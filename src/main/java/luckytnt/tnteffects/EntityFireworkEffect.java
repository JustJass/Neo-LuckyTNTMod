...
public class EntityFireworkEffect extends PrimedTNTEffect {
	...
	public void serverExplosion(IExplosiveEntity ent) {
		EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.get(ResourceLocation.parse(ent.getPersistentData().getString("type")));
		if(type == null) {
			type = EntityType.PIG;
		}
		for(int count = 0; count < 300; count++) {
			...
			if(lent instanceof Mob mob && ent.getLevel() instanceof ServerLevel sLevel) {
				mob.finalizeSpawn(sLevel, sLevel.getCurrentDifficultyAt(toBlockPos(ent.getPos())), MobSpawnType.MOB_SUMMONED, null);
			}
